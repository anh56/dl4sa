...
def generate_features(x_train, x_val, x_test, feature='bow'):
    ...
    if feature == 'bow':
        ...
        print("len(slt_char_vocabs)", len(slt_char_vocabs))
        ...
        print("len(model.wv)", len(model.wv))
...
def main(granularity, cvss_col, feature):
    ...
    print(df[df[cvss_col].isnull()].index.tolist())
    ...
    print("len(x_train)", len(x_train), "Counter(y_train)", Counter(y_train))
